...
class MainViewModel : ViewModel() {
    ...
    fun retrieveData(userId: String) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            try {
                data.value = ProductApi.service.getProduct(userId)
    ...
    fun saveData(userId: String, product: String, price: String, bitmap: Bitmap) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val result = ProductApi.service.postProduct(
                    userId,
                    product.toRequestBody("text/plain".toMediaTypeOrNull()),
    ...
    fun deleteProduct(userId: String, id: String) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val result = ProductApi.service.deleteProduct(userId, id)
                if (result.status == "success") {
                    retrieveData(userId)
                } else {
                    throw Exception(result.message)
